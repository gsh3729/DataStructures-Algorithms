	...
	while(1)
	{
		...
		switch(option)
		{
			...
			case  6 :
				...
				{
					if(findInorderPrecedor(root,item )!=NULL)
					{
						printf("PreceedingKey of %d is %d\n",item,findInorderPrecedor(root,item )->key);
					}
...
void deleteNode(node **root,int key)
{
	...
	if (key == (*root)->key)
	{
		if((*root)->leftptr==NULL && (*root)->rightptr==NULL)//no child
		{
			...
			{
				(*root)->parentptr->leftptr=NULL;
			}
			else if(key > (*root)->parentptr->key)
			{
				(*root)->parentptr->rightptr=NULL;
			}
			
			free(temp);
			return ;
		}
		if((*root)->leftptr==NULL && (*root)->rightptr!=NULL)//only rightchild
		{
			if((*root)->parentptr == NULL)
			{
				node *temp1=*root;
		...
		if((*root)->leftptr!=NULL && (*root)->rightptr==NULL)//only leftchild
		{
			if((*root)->parentptr == NULL)
			{
				node *temp1=*root;
